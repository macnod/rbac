...
(defparameter *create-id* (get-id *rbac* "permissions" "create"))
(defparameter *delete-id* (get-id *rbac* "permissions" "delete"))
(defparameter *read-id* (get-id *rbac* "permissions" "read"))
(defparameter *admin-role-id* (get-id *rbac* "roles" "admin"))
(defparameter *editor-role-id* (get-id *rbac* "roles" "editor"))
...
(test next-placeholder-test
  (is (= (sql-next-placeholder "select ... where c = $1") 2)
    "select ... where c = $1 => 2")
  (is (= (sql-next-placeholder "select ... where c = $1 and d = $2") 3)
    "select ... where c = $1 and d = $2 => 3")
  (is (= (sql-next-placeholder "select ... where c = $2 and d = $1") 3)
    "select ... where c = $2 and d = $1 => 3")
  (is (= (sql-next-placeholder "select ... where c = 1 and d = 2") 1)
    "select ... where c = 1 and d = 2 => 1")
  (is (= (sql-next-placeholder "$5") 6)
    "$5 => 6")
  (is (= (sql-next-placeholder "") 1)
    "(empty string) => 1")
  (is (= (sql-next-placeholder nil) 1)
    "NIL => 1"))
...
(defun get-resources (&optional part)
  (let ((resources (list-resources *rbac* 1 1000)))
    (if part
